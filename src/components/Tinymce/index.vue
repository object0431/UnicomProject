...
<script>
 ...
 `advlist anchor autolink autosave code codesample colorpicker colorpicker
  ...
  alignright outdent indent  blockquote undo redo removeformat code`,
    `hr bullist numlist link  charmap	 preview anchor pagebreak
    fullscreen insertdatetime  table forecolor backcolor`
  ];
  export default {
    name: 'tinymce',
    props: {
    ...
    watch: {
      value(val) {
        if (!this.hasChange && this.hasInit && val !== '' && val !== null && val !== undefined) {
